...
async def force_sub(c, m):
    if Config.FORCE_SUB:
        try:
            ...
            logger.warning(f"Make me admin in @Deva_TG_Update")
            if m.from_user.id in Config.AUTH_USERS:
                return await m.reply_text(f"Make me admin in @Deva_TG_Update")
            ...
            logger.error(e)
            return await m.reply_text("Some thing went wrong. Try again and if same issue occur contact [our Channel](https://t.me/Deva_TG_Update)", disable_web_page_preview=True, quote=True)
